...
const axios = require("axios");  // axios is a library for sending/receiving https requests in a more modern way
...
describe("metadata microservice", () => {
    ...
    let microservice; // Saves a reference to the our microservice object. this should be a var...

    // these two functions below will run before/after all tests in this suite, respectively
    beforeAll(async () => {
    ...
    // aka populate the db with some pre-defined tables/data for testing purposes
    async function loadDatabaseFixture(collectionName, records) {
        await microservice.db.dropDatabase(); // Reset the test database. don't do this in the real world!!!
    ...
    test("/videos route retrieves data via videos collection", async () => {
        ...
        const videoPath1 = "my-video-1.mp4"; // replace these with actual vids?
        const videoPath2 = "my-video-2.mp4"; // actually no, don't do that. this is just a test
